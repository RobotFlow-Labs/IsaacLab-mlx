...
"""Smoke tests for the mac-native Franka cabinet slice."""
...
def test_mac_franka_cabinet_reset_and_step_shapes():
    """The Franka cabinet env should expose deterministic IsaacLab-style tensors."""

    cfg = MacFrankaCabinetEnvCfg(num_envs=8, seed=73, episode_length_s=0.5)
    ...
    assert env.sim_backend.state_dict()["task"] == "franka-cabinet"


def test_mac_franka_cabinet_can_open_drawer_when_handle_is_grasped():
    """Pulling along +X with the gripper closed should open the drawer and terminate successfully."""

    cfg = MacFrankaCabinetEnvCfg(num_envs=2, seed=79, episode_length_s=0.5)
    env = MacFrankaCabinetEnv(cfg)
...
def test_mac_franka_cabinet_rollout_replay_is_deterministic():
    """Rollout/replay helpers should preserve Franka cabinet trajectories for fixed actions."""

    cfg = MacFrankaCabinetEnvCfg(num_envs=4, seed=83, episode_length_s=0.5)
    actions = mx.zeros((cfg.num_envs, cfg.action_space), dtype=mx.float32)

    env_a = MacFrankaCabinetEnv(cfg)
    trace_a = rollout_env(env_a, actions, steps=6)

    env_b = MacFrankaCabinetEnv(cfg)
...
def test_train_and_play_franka_cabinet_smoke(tmp_path: Path):
    """A tiny MLX train/play loop should produce a reusable Franka cabinet checkpoint."""

    checkpoint_path = tmp_path / "franka_cabinet_policy.npz"
    train_cfg = MacFrankaCabinetTrainCfg(
        env=MacFrankaCabinetEnvCfg(num_envs=8, seed=89, episode_length_s=0.5),
        hidden_dim=32,
        updates=1,
        rollout_steps=8,
        epochs_per_update=1,
    ...
    result = train_franka_cabinet_policy(train_cfg)
    ...
    assert metadata["task_id"] == "Isaac-Franka-Cabinet-Direct-v0"
    ...
    episode_returns = play_franka_cabinet_policy(
        str(checkpoint_path),
        env_cfg=MacFrankaCabinetEnvCfg(num_envs=1, seed=89, episode_length_s=0.5),
        episodes=1,
    )
...
def test_train_franka_cabinet_resume_uses_checkpoint_hidden_dim(tmp_path: Path):
    """Resuming Franka cabinet should preserve the checkpoint architecture."""

    first_checkpoint = tmp_path / "franka_cabinet_policy_initial.npz"
    resumed_checkpoint = tmp_path / "franka_cabinet_policy_resumed.npz"

    initial_result = train_franka_cabinet_policy(
        MacFrankaCabinetTrainCfg(
            env=MacFrankaCabinetEnvCfg(num_envs=8, seed=97, episode_length_s=0.5),
            hidden_dim=32,
            updates=1,
            rollout_steps=8,
            epochs_per_update=1,
    ...
    resumed_result = train_franka_cabinet_policy(
        MacFrankaCabinetTrainCfg(
            env=MacFrankaCabinetEnvCfg(num_envs=8, seed=97, episode_length_s=0.5),
            updates=1,
            rollout_steps=8,
