...
MAC_NATIVE_TASK_SPECS: tuple[MacNativeTaskSpec, ...] = (
    ...
    MacNativeTaskSpec(
        key="factory-gear-mesh",
        upstream_task_id="Isaac-Factory-GearMesh-Direct-v0",
        family="manipulation",
        benchmark_groups=("manipulation-expansion",),
        sensor_contract=("proprioception",),
        trainable=True,
        default_checkpoint="logs/mlx/factory_gear_mesh_policy.npz",
        default_hidden_dim=128,
        default_action_std=0.2,
        semantic_contract="reduced-analytic-gear-mesh",
        upstream_alias_semantics_preserved=False,
        notes=(
            "Reduced analytic factory gear-mesh slice preserving the alignment and insertion workflow "
            "with scalar insertion progress instead of the upstream contact-rich factory scene and task-specific controller stack."
        ),
    ),
    MacNativeTaskSpec(
