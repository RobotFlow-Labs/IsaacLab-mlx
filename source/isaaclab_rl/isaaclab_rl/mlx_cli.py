...
"""Installed CLI surface for the MLX/mac-sim task runners."""
...
def _train_from_args(args: argparse.Namespace, default_task: str | None) -> int:
...
def _eval_from_args(args: argparse.Namespace, default_task: str | None) -> int:
    task = default_task or args.task
...
def run_train_cli(default_task: str | None = None) -> int:
    return _train_from_args(_build_train_parser(default_task).parse_args(), default_task)


def run_eval_cli(default_task: str | None = None) -> int:
    return _eval_from_args(_build_eval_parser(default_task).parse_args(), default_task)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if argv:
        if argv[0] == "train":
            return _train_from_args(_build_train_parser(None).parse_args(argv[1:]), None)
        if argv[0] == "evaluate":
            return _eval_from_args(_build_eval_parser(None).parse_args(argv[1:]), None)
    parser = argparse.ArgumentParser(description="Installed CLI surface for MLX/mac-sim task training and replay.")
    parser.add_argument("command", choices=("train", "evaluate"), help="CLI command to run.")
