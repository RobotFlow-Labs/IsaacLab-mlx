...
- `DONE` Planner/ROS process manifests now aggregate batch transcript paths plus replay metadata, giving the plain-data bridge a deterministic session-level audit trail without importing ROS Python bindings
- `DONE` Planner/ROS replay helpers now reconstruct ordered command groups from session-manifest transcript paths so the plain-data bridge can audit deterministic replay without ROS Python bindings
...
- `DONE` Upstream-compatible Franka lift IK variants now resolve to the canonical mac-native lift slice, and the reduced-contract Franka stack blueprint/skillgen/visuomotor/cosmos aliases plus `Isaac-Deploy-Reach-UR10e-ROS-Inference-v0` now resolve explicitly to existing reduced canonical slices on mac instead of pretending full subsystem parity; `Isaac-Stack-Cube-Bin-Franka-IK-Rel-Mimic-v0` remains the reduced-contract supported exception
...
- `DONE` Five more trainable reduced manipulation slices landed together for `Isaac-Reach-OpenArm-v0`, `Isaac-Reach-OpenArm-Bi-v0`, `Isaac-Reach-UR10-v0`, `Isaac-Lift-Cube-OpenArm-v0`, and `Isaac-Open-Drawer-OpenArm-v0`, with honest reduced-contract metadata, public MLX wrapper/CLI support, benchmark coverage, semantic baseline refresh to `task_count=26`, and a broad backend regression ring at `324 passed, 21 warnings`
...
- `DONE` Two new trainable reduced Agibot placement slices landed for `Isaac-Place-Toy2Box-Agibot-Right-Arm-RmpFlow-v0` and `Isaac-Place-Mug-Agibot-Left-Arm-RmpFlow-v0`, with a shared analytic placement substrate, honest `semantic_contract="reduced-agibot-place-surrogate"` metadata across the lazy registry and public MLX wrapper, direct thin train/play wrappers, refreshed benchmark signatures, a deterministic semantic baseline refresh to `task_count=32`, and a broad backend regression ring at `386 passed, 21 warnings`
- `DONE` One new honest reduced manipulation slice landed for `Isaac-Factory-PegInsert-Direct-v0` on the shared analytic insertion substrate, with explicit `semantic_contract="reduced-analytic-peg-insert"` metadata across the lazy registry and public MLX wrapper, benchmark coverage in the `manipulation-expansion` group, checkpoint/replay support, and focused registry/wrapper/CLI validation; the task stays outside `current-mac-native` so the stable core rollout surface does not overclaim parity
- `DONE` One more honest reduced manipulation slice landed for `Isaac-Factory-GearMesh-Direct-v0` on the shared analytic insertion substrate, with explicit `semantic_contract="reduced-analytic-gear-mesh"` metadata across the lazy registry and public MLX wrapper, dedicated gear-mesh benchmark signatures, semantic baseline refresh to `task_count=34`, and a broad backend regression ring after merge; the task also stays in `manipulation-expansion` instead of inflating the stable core rollout surface
...
- Port the next genuinely new manipulation milestone beyond the current fifteen trainable manipulation slices, likely a richer non-Franka cabinet/drawer variant or the next multi-object workflow that cannot be represented honestly as an alias of the existing reduced slices.
...
- Keep manipulation compatibility aliasing honest: widen upstream task-ID coverage only where the reduced mac-native slice still matches the observation/action/checkpoint contract, and if the upstream name encodes an extra policy/model/process layer, either keep it gated or normalize it as an explicit reduced alias with `semantic_contract`, `contract_notes`, and unchanged public task counts.
- The next manipulation milestone should stay genuinely new. The honest alias/gating boundary for the current Franka family now includes the separate instance-randomized stack and bin-stack slices, so further progress should come from another reduced mac-native task rather than re-aliasing richer upstream families.
