...
- `DONE` Upstream-compatible Franka lift IK variants now resolve to the canonical mac-native lift slice, while richer Franka stack visuomotor/cosmos/blueprint/skillgen families remain discoverable through explicit `sim-backend=isaacsim` gating on mac; `Isaac-Stack-Cube-Bin-Franka-IK-Rel-Mimic-v0` is the reduced-contract supported exception
