...
"""Run the mac-native ANYmal-C flat slice with random, zero, or checkpointed actions."""
...
from isaaclab.backends.mac_sim import MacAnymalCFlatEnv, MacAnymalCFlatEnvCfg, play_anymal_c_policy


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Play the MLX/mac-sim ANYmal-C flat slice.")
...
def main() -> int:
    ...
    if args.checkpoint:
        returns = play_anymal_c_policy(args.checkpoint, env_cfg=cfg, episodes=args.episodes)
        for index, episode_return in enumerate(returns, start=1):
            print(f"[mlx-anymal-c-flat] episode={index} return={episode_return:.4f}")
        if len(returns) < args.episodes:
            print(
                "[mlx-anymal-c-flat] completed fewer episodes than requested "
                f"({len(returns)}/{args.episodes})"
            )
        return 0
    ...
    for index, (length, episode_return) in enumerate(completed[: args.episodes], start=1):
        print(f"[mlx-anymal-c-flat] episode={index} length={length} return={episode_return:.4f}")
